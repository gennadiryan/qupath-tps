...
public class ServerOverlay extends AbstractOverlay {
	...
	private static Logger logger = LoggerFactory.getLogger(ImageCombinerWarpyServerOverlay.class);
    ...
    private BooleanProperty isOpacityProperty;
	 ...
	 */
	public ServerOverlay(final QuPathViewer viewer, final ImageServer<BufferedImage> server) {
	 ...
	 */
	public ServerOverlay(final QuPathViewer viewer, final ImageServer<BufferedImage> server, final Affine affine) {
		...
		updateTransform();
        this.isOpacityProperty = new SimpleBooleanProperty(false);
        this.opacityProperty = new SimpleDoubleProperty(1.0);
	}
    ...
    public AlphaComposite getComposite() {
        return AlphaComposite.getInstance(AlphaComposite.SRC_OVER, (float) this.opacityProperty.get());
    }

    public void setComposite(Graphics2D g2d) {
        if (this.isOpacityProperty.get())
